...
docker exec -i $MYSQL_CONTAINER_NAME /bin/bash -c "mysql -u root -p$MYSQL_ROOT_PASSWORD -e 'CREATE DATABASE IF NOT EXISTS $MYSQL_DATABASE; USE $MYSQL_DATABASE; source /var/lib/mysql/backup.sql;'"
