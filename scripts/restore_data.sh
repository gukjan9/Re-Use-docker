...
docker exec -i $MYSQL_CONTAINER_NAME /bin/bash -c "mysql -u root -p$MYSQL_DATABASE_PASSWORD -e 'CREATE DATABASE IF NOT EXISTS ${MYSQL_DATABASE} DEFAULT CHARACTER SET UTF8; USE ${MYSQL_DATABASE};' && mysql -u root -p$MYSQL_DATABASE_PASSWORD $MYSQL_DATABASE < /var/lib/mysql/backup.sql"
